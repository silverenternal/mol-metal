...
def _maybe_fused_residual_add(
    ...
    *,
    ...
    The fused kernel implements ``out = alpha * x + beta * residual`` in a
    single HBM round-trip, and the
    ...
    via :data:`triton_config` (which respects ``TRITON_USE_FUSED`` and the
    size/training-mode policy).

    The cost saved is one HBM round-trip per EGNN layer: the standard
    ResNet-style skip connection ``h + update`` materialises ``update``
    to HBM and re-reads both ``h`` and ``update`` before writing the
    sum; the fused kernel keeps the addition in-register.

    Lit anchors (kernel design):
        ...
        - He 2016 "Identity Mappings in Deep Residual Networks" — the
          post-norm residual pattern that this kernel implements.
    """
    if triton_config.use_fused_residual_add(x.numel()):
        return _triton_fused_residual_add(x, residual, alpha=alpha, beta=beta)
    # Fallback: pure-PyTorch ``torch.add`` with ``alpha`` (matches
    # bit-for-bit when ``alpha == 1``; explicit scale otherwise).
...
class EGNNLayer(nn.Module):
    ...
    def forward(
        ...
        # Opt-in architectural bound for molecular flow sampling. This
        # bounds the learned scalar coefficient, not generated coordinates.
        bounded_vectors = getattr(self, "bounded_coordinate_messages", False)
        if bounded_vectors:
            phi = torch.tanh(phi)
        ...
        # The wrapper at module scope gates on triton_config + size.
        h_next = _maybe_fused_residual_add(h_node, update)
        return h_next, v_agg
...
class VelocityNet(nn.Module):
    ...
    def forward(
        ...
        if cond is not None and self.cond_proj is not None:
            ...
            # Fused residual add (Wang 2020 Triton kernel) — the cond
            # projection residual that injects time embedding into every
            # layer.  Saves one HBM round-trip per forward.
            cond_per_node = _maybe_fused_residual_add(cond_per_node, t_per_node)
        else:
