...
class _TranspositionTable:
    ...
    already been inserted (via a different reaction path or a parallel
    worker), the table returns the existing node so PUCT statistics
    accumulate instead of fragmenting into duplicate subtrees.  This
    is the standard transposition-table hook used in AlphaGo-style
    lock-free MCTS.
...
class _MCTSNode:
    ...
    def is_terminal(self) -> bool:
        """True iff the state is in β-normal form AND closed.

        Such a state cannot undergo any further β-reduction (= no
        reaction can fire), so MCTS should not expand from it.
        """
        s = self.state
        try:
            return bool(s.is_beta_normal_form)
        except Exception:
            return False

    @property
    def is_closed_for_search(self) -> bool:
        """True iff the node should *terminate* the SELECT descent.

        A node is "closed for search" only when it is in β-NF *and* has
        no children already added to the tree (= no rule-fired
        descendants to descend into).  This is the gate MCTS should
        consult: if a node has children added (e.g. by the root
        Dirichlet expansion), the SELECT loop must descend through it
        even when its own state is β-NF.

        Returns ``False`` if the node has rule-fired children, so the
        SELECT loop keeps descending past β-NF roots.
        """
...
class MCTSProofSearch:
    ...
    def _simulate(
        ...
        # The gate is "closed for search": a node is only a stopping
        # point when its state is β-NF *and* it has no rule-fired
        # children already on the tree.  This is what lets the SELECT
        # loop descend past a β-NF root that has been expanded via
        # ``_apply_dirichlet_to_root`` (or any other prior
        # expansion), driving non-zero ROLLOUT_DEPTH.
        node = root
        ...
        if not node.is_terminal and len(path) <= max_depth:
            ...
            if children:
                ...
                for child_state, rule_name, tile in child_states:
                    if self._node_already_present(node, child_state):
                        continue
                    # (D2) Transposition-table lookup: if ``child_state``
                    # has already been inserted elsewhere in the tree
                    # under an equivalent canonical SMILES, re-use the
                    # existing node rather than allocating a fresh one.
                    # We re-parent it to ``node`` so PUCT statistics
                    # remain valid for both descent paths.
                    child_smi = self._canonical_smi(child_state)
                    existing, hit = self._lookup_or_create(
                        child_smi,
                        factory=lambda: _MCTSNode(
                            state=child_state,
                            parent=node,
                    ...
                    if hit:
                        # Existing node — just re-parent to current
                        # expansion root and accumulate the PUCT path.
                        try:
    ...
    def _apply_dirichlet_to_root(
        ...
        for child_state, rule_name, tile in root_children:
            if self._node_already_present(root, child_state):
                continue
            # (D2) Transposition-table integration — reuse an existing
            # equivalent node if one is registered, otherwise allocate
            # a fresh child and register it.
            child_smi = self._canonical_smi(child_state)
            existing, hit = self._lookup_or_create(
                child_smi,
                factory=lambda: _MCTSNode(
    ...
    def _rollout(
        ...
        while cur_depth < depth:
            # Stop if we reached β-NF (no more reactions can fire).
            try:
                if current.is_beta_normal_form:
                    break
            except Exception:
                pass
    ...
    def _collect_leaves(
        ...
        """Walk the tree and update ``leaves_by_smi`` with every leaf seen."""
        ...
        while stack:
            ...
            if not n.children:
                ...
                if key in leaves_by_smi:
                    # Merge statistics: sum N and W into the existing
                    # entry.  (Avoids losing data when MCTS rediscovers
                    # an α-equivalent state via a different path.)
                    existing = leaves_by_smi[key]
                    existing.N += n.N
                    existing.W += n.W
                else:
                    leaves_by_smi[key] = n
