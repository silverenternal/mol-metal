...
class RewardAggregator:
    ...
    #: Phase 3H — soft, continuous metal-geometry prior channel.
    #: Replaces the historical hard-gate ``metal_geometry_prior_bonus``
    #: (binary 0/1 depending on whether CN matched the target) with a
    #: continuous distance-to-ideal score in ``[0, 1]``.  When supplied,
    #: this callable is invoked with the molecule and should return a
    #: float in ``[0, 1]``.  Default impl wraps
    #: :func:`molmetal_lam.priors.metal_geometry.soft_score_metal_geometry`.
    #: When the molecule has no metal centre the channel returns 0.0
    #: (organic ligands don't earn a metal prior bonus by construction).
    r_metal_geom_soft: Optional[Callable[[MoleculeClosedTerm], float]] = None
    #: Phase 2 (Metallodrug ultracode) — PlatinAI predicted-activity
    #: oracle channel.  Returns a kNN-weighted mean of A2780 (ovarian)
    #: + MCF7 (breast) predicted probabilities for the 5 nearest
    #: neighbours in the PlatinAI_MBFinder corpus (226,918 SMILES,
    #: Morgan-ECFP4 Tanimoto).  Falls back to 0.0 when no neighbour
    #: is within Tanimoto 0.3 of the query.  Default impl wraps
    #: :class:`molmetal_lam.reward.platinai_oracle.PlatinAIOracle`.
    #: When the corpus xlsx files are missing or RDKit cannot parse
    #: the molecule the channel silently degrades to 0.0 — never
    #: crashes.  Lit anchor: Riniker 2013 (kNN oracle + Tanimoto as
    #: activity-similarity proxy) and Baskin 2017 (kNN-based
    #: bioactivity scoring for cheminformatics).
    r_platinai: Optional[Callable[[MoleculeClosedTerm], float]] = None
    #: T24 (2026-09-17) — RxnFlow template-match oracle channel
    #: (cite-only fallback).  Returns 1.0 if the candidate matches the
    #: product SMARTS of the named Enamine REAL template, else 0.0.
    #: Falls back to 0.0 when RxnFlow upstream is not installed or when
    #: the RDKit template-match cannot be evaluated.  Lit anchor: Seo
    #: 2024 arXiv:2410.04542 (RxnFlow / RxnFlowSampler) and the 109
    #: Enamine REAL templates shipped in
    #: ``molmetal/references/RxnFlow/data/templates/real.txt``.  This is
    #: the cheapest possible oracle — a real RxnFlow evaluation would
    #: sample from the GFlowNet and compute the template-conditional
    #: reward; we only do a product-side substructure match so the
    ...
    #: TODO-30 P2.5 Phase A (2026-09-17) — pharmacophore pre-filter
    #: channel.  Returns a continuous pharmacophore score in ``[0, 1]``
    #: (1.0 = strict Lipinski+Veber+ring-count pass, 0.0 = all 7
    #: violations including parse-failure).  Default impl wraps
    #: :func:`molmetal_lam.lam_chem.pharmacophore_filter.pass_pharmacophore`
    #: (boolean, leniency auto-mapped to a 0.0/1.0 score) plus a
    #: partial-credit scoring function
    #: (:func:`compute_pharmacophore_score`) that maps
    #: ``total_violations / 7`` to ``1 - violations/7``.  Falls back
    #: to ``0.0`` on any exception (parse-failure, RDKit missing,
    #: etc.) so the aggregator degrades silently — never crashes
    #: MCTS.  Lit anchor: Lipinski 2001 (Ro5), Veber 2002
    #: (PSA + rotB), Hopkins 2008 (ring-count >= 1).  Distinct from
    #: the historical :func:`pass_pharmacophore` *gate* which is
    #: boolean and used in the pipeline pre-filter; this channel is
    #: a continuous *reward signal* suitable for additive MCTS
    #: shaping.
    r_pharmacophore: Optional[Callable[[MoleculeClosedTerm], float]] = None
    #: TODO-30 P5.2 Tier 1 (2026-09-17) — wet-lab signed-error channel.
    #: Returns ``-|predicted - measured|`` for assayed candidates,
    #: ``0.0`` otherwise.  Default impl wraps
    #: :func:`molmetal_lam.lam_chem.wetlab_reward_channel.compute_wetlab_signed_error`
    #: via :meth:`register_wetlab_channel`.  The channel gracefully
    #: degrades to 0.0 on parse failure or missing measurement — never
    #: crashes MCTS.  When ``measurement_dict`` is ``None`` the closure
    #: is a permanent no-op (still safe to wire; default weight 0.0
    #: keeps the historical reward bit-for-bit identical when unused).
    r_wetlab: Optional[Callable[[MoleculeClosedTerm], float]] = None
    #: TODO-15 — equal-weight anticancer_index channel (logP window +
    #: metal coordination + GSH evasion + DNA Kb proxy).  Distinct
    #: from ``r_anticancer_composite`` (legacy 6-channel weighted sum
    #: from the priors module).  When wired by
    #: :meth:`register_anticancer_channels` and the suite has the
    #: ``composite_score -> dict`` API, the channel returns the
    #: ``anticancer_index`` key.
    r_anticancer_index: Optional[Callable] = None
    w_vina: float = 1.0
    w_sa: float = 1.0
    w_qed: float = 1.0
    ...
    #: TODO-15 weight on the equal-weight anticancer_index channel.
    #: Defaults to 0.0 (opt-in) so existing per-channel reward is
    #: unchanged; set ``w_anticancer_index = 1.0`` to make the
    #: anticancer_index contribute on the same footing as the other
    #: channels.
    w_anticancer_index: float = 0.0
    #: Phase 3H — weight on the soft, continuous metal-geometry channel.
    #: Defaults to 0.0 (opt-in) so existing reward is bit-for-bit
    #: unchanged when unused; set ``w_metal_geom_soft = 1.0`` (or any
    #: positive scalar) to make the soft prior contribute on the same
    #: footing as the other reward channels.  Lit anchors for the soft
    #: formulation are Schulman 2017 PPO (avoid hard 0/-inf rewards),
    #: Neu 2017 (entropy-regularised MDPs prefer soft preferences) and
    #: Dayan 1997 (potential-based reward shaping is policy-preserving).
    w_metal_geom_soft: float = 0.0
    #: Phase 2 (Metallodrug ultracode) — weight on the PlatinAI
    #: predicted-activity oracle channel.  Defaults to 0.0 (opt-in)
    #: so existing reward is bit-for-bit unchanged when unused; set
    #: ``w_platinai = 1.0`` (or any positive scalar) to make the
    #: oracle contribute on the same footing as the other reward
    #: channels.  The oracle already returns values in [0, 1] so the
    #: scalar weight maps directly to the aggregated reward.
    w_platinai: float = 0.0
    #: T24 weight on the RxnFlow template-match oracle channel.
    #: Defaults to 0.0 (opt-in) so existing weighted reward is bit-for-
    #: bit unchanged when unused; set ``w_rxnflow = 1.0`` to make the
    #: template-match contribute on the same footing as the other
    #: reward channels.  When the candidate matches the named template's
    #: product SMARTS the channel contributes ``w_rxnflow * 1.0``; when
    #: it does not match (or the upstream is unavailable) it
    #: contributes ``0.0``.
    w_rxnflow: float = 0.0
    #: TODO-30 P2.5 Phase A weight on the pharmacophore filter
    #: channel.  Defaults to 0.0 (opt-in) so existing weighted
    #: reward is bit-for-bit unchanged when unused; set
    #: ``w_pharmacophore = 1.0`` (or any positive scalar) to make
    #: the continuous pharmacophore score contribute on the same
    #: footing as the other reward channels.  Because the channel
    #: returns ``1 - violations/7`` (i.e. a value in [0, 1] where
    #: 1.0 is strict pass), a unit weight adds at most ``+1.0`` to
    #: the leaf value — well-bounded against the dominant Vina /
    #: SA reward channels.
    w_pharmacophore: float = 0.0
    #: TODO-30 P5.2 Tier 1 weight on the wet-lab signed-error channel.
    #: Defaults to 0.0 (opt-in) so existing weighted reward is
    #: bit-for-bit unchanged when unused; set ``w_wetlab = 1.0`` to
    #: make the signed-error term contribute on the same footing as
    #: the other reward channels.  Because the channel returns a
    #: value in ``[-3, 0]`` (clamped), the additive contribution is
    #: bounded by ``3 * w_wetlab`` per leaf (i.e. at most ``-3.0``
    #: when weight=1.0).
    w_wetlab: float = 0.0
    vina_invert: bool = True
    bonus_typed: float = 0.5
    bonus_binder: float = 0.5
    #: WF-Lambda-Fix-FullPath Fix 3 — diversity bonus channel.  At
    #: rank-time :meth:`MCTSProofSearch.search` re-evaluates each
    #: candidate's bonus contribution as ``+w_diversity`` if its
    #: canonical SMILES differs from every OTHER candidate in the
    #: result set, ``0.0`` otherwise.  This biases the final top-K
    #: ordering toward molecules that are *uniquely* represented in
    #: the result set, breaking ties when reward scores are similar.
    #: Defaults to ``0.0`` (off) so existing scalar-reward behaviour
    #: is bit-for-bit unchanged when unused; set ``w_diversity = 0.1``
    #: in production pilots to lift :attr:`diversity_tanimoto` /
    #: :attr:`diversity_homotype` without disturbing the dominant
    #: Vina / SA / PB reward channels.
    w_diversity: float = 0.0
    #: WF-Deflex Wire-up Phase 1 — F5 symbolic-reward shaping channel.
    #: When ``use_learned_shaping=True`` (or env-var
    #: ``LEARNED_SHAPING_ENABLED=1`` is set), ``__call__`` applies the
    #: F5 formula ``R_F5(sa_score) = 2.5836 - 2.5149 * sa_score`` to the
    #: current SA channel and adds ``w_learned_shaping * R_F5(sa)`` to
    #: the aggregate.  Defaults to ``False`` so existing scalar reward
    #: is bit-for-bit identical when unused.  The channel is a thin
    #: wrapper over
    #: :func:`molmetal_lam.reward.learned_shaping.LearnedShaping.shape_reward`
    #: so callers wanting a re-fit can pass a custom formula via
    #: :meth:`register_learned_shaping_channel`.  Lit anchor: McAllester
    #: 1999 PAC-Bayes regularised linear (sparse in-sample fit on n=33
    #: MEASURED cells picked sa_norm as the lone surviving signal).
    use_learned_shaping: bool = False
    #: Weight on the F5 channel.  Defaults to ``1.0`` so a single call
    #: with ``use_learned_shaping=True`` flips the F5 contribution on
    #: at full magnitude; pass ``0.0`` to leave the channel installed
    #: but disabled.  When ``use_learned_shaping=False`` the weight is
    #: ignored (channel is a no-op even if non-zero).
    w_learned_shaping: float = 1.0
    #: F5 env-var name (read lazily at ``__call__`` time so callers can
    #: flip the gate at runtime and see the effect on the next MCTS
    #: iteration).  Mirrors ``LearnedShaping._ENABLED_ENV``.
    _LEARNED_SHAPING_ENV: str = "LEARNED_SHAPING_ENABLED"

    # ------------------------------------------------------------------
    # Diversity bonus (Fix 3) — rank-time helper
    # ------------------------------------------------------------------
    def diversity_bonus(
        self,
        state: MoleculeClosedTerm,
        result_set: Sequence[MoleculeClosedTerm],
    ) -> float:
        """Return ``1.0`` if ``state`` is unique within ``result_set``,
        else ``0.0``.

        WF-Lambda-Fix-FullPath Fix 3 — closes the singleton-collapse
        failure mode where MCTS re-emits the seed or a single click
        product across many rollouts.  The bonus is computed at
        rank-time (after the MCTS loop has produced the result set)
        by comparing the candidate's canonical SMILES against every
        other member of ``result_set``.
        ...
        result_set : Sequence[MoleculeClosedTerm]
            The other candidates in the result set.  ``state`` itself
            MAY appear in ``result_set``; the comparison excludes the
            candidate by index.  An empty ``result_set`` returns
            ``1.0`` (vacuously unique).

        Returns
        -------
        float
            ``1.0`` if no *other* member of ``result_set`` shares the
            candidate's canonical SMILES; ``0.0`` otherwise.  A
            candidate that fails to produce a canonical SMILES is
            treated as non-unique (returns ``0.0``).
        """
        try:
        ...
        if not cand_smi:
            return 0.0
        for i, other in enumerate(result_set):
            if other is state:
                continue
            try:
                other_smi = other.canonical_smiles() or ""
            except Exception:
                continue
            if other_smi and other_smi == cand_smi:
                return 0.0
        return 1.0
    ...
    def register_anticancer_channels(self, suite) -> None:
        ...
        Also wires the new (TODO-15) :attr:`r_anticancer_index`
        channel which exposes the equal-weight
        ``anticancer_index`` from
        :class:`molmetal_lam.metrics.anticancer_metric_suite.AnticancerMetricSuite.composite_score`.
        Falls back gracefully to the existing
        :attr:`r_anticancer_composite` channel when the suite is the
        legacy ``priors.anticancer_metric_suite.AnticancerMetricSuite``
        that lacks ``composite_score(mol) -> dict``.
        """
        def _smiles(state):
        ...
        # TODO-15: equal-weight anticancer_index channel.  The new
        # metric suite returns a dict so we read the "anticancer_index"
        # key; if the suite returns a float (legacy API) we reuse it.
        def _anticancer_index(state):
            result = suite.composite_score(_smiles(state))
            if isinstance(result, dict):
                return float(result.get("anticancer_index", 0.0))
            return float(result)
        self.r_anticancer_index = _anticancer_index

    def register_soft_metal_geometry_channel(
        self,
        *,
        enabled: bool = True,
        cn_targets: Optional[dict] = None,
        weight_coord: float = 0.5,
        weight_angle: float = 0.3,
        weight_charge: float = 0.2,
        ideal_charge: int = 0,
    ) -> None:
        """Wire the ``r_metal_geom_soft`` channel to :func:`soft_score_metal_geometry`.

        Phase 3H — replaces the historical hard-gate
        ``metal_geometry_prior_bonus`` (binary 0/1 depending on whether
        CN matched the target) with a continuous distance-to-ideal
        score in ``[0, 1]``.  Lit anchors: Schulman 2017 (avoid hard
        0/-inf rewards via ratio clipping), Neu 2017 (entropy-
        regularised MDPs prefer soft preferences), Dayan 1997
        (potential-based reward shaping is policy-preserving).

        Parameters
        ----------
        enabled : bool, default True
            When False the channel returns 0.0 unconditionally — matches
            the historical "prior off" ablation path (Round-10 axis D).
        ...
        Notes
        -----
        This method does NOT mutate :attr:`w_metal_geom_soft` — the
        caller remains in control of the per-channel weight.  Default
        is ``0.0`` (off) so existing reward is bit-for-bit identical
        when unused; set ``w_metal_geom_soft = 1.0`` in production
        pilots to enable the soft prior.
        """
        # Import lazily to keep the proof_search import surface lean
        # and to avoid a circular import (priors.metal_geometry imports
        # nothing from search_alg).
        from molmetal_lam.priors.metal_geometry import (
            make_soft_metal_geometry_channel,
        )
        self.r_metal_geom_soft = make_soft_metal_geometry_channel(
            enabled=enabled,
    ...
    def register_reinvent4_multiproperty_channel(
        self,
        adapter: "Any",
        *,
        default_components: Optional[Dict[str, float]] = None,
    ) -> None:
        """Wire the ``r_reinvent4`` channel to a :class:`REINVENT4MultipropertyAdapter`.

        WF-Extra-2 closes the historical "r_reinvent4 silently off" gap:
        before this method existed, callers had to hand-roll a closure
        that knew how to call the subprocess adapter.  This helper
        standardises the contract so a single call wires the channel,
        the timeout, and the graceful-degradation path.

        Behaviour:

        * When the adapter is unavailable (``adapter.available`` is
          ``False``), ``r_reinvent4`` is left as ``None`` so the
          aggregator's existing graceful-fail path emits ``0.0`` for
          the channel (no spurious exceptions, no monkey-patched
          ``_safe`` wrapper needed).
        * When the adapter IS available, the channel is set to a
          closure that:
            1. Extracts a SMILES string from the molecule state
               (accepts both ``MoleculeClosedTerm`` objects and plain
               ``str`` inputs);
            2. Calls ``adapter.score([smiles])[0]`` — the multiproperty
               adapter always returns a ``list[float]``;
            3. Returns ``0.0`` on any exception, leaving
               ``adapter.last_error`` populated for diagnostics.

        Parameters
        ----------
        adapter : REINVENT4MultipropertyAdapter
            The subprocess-RPC adapter (see
            :class:`molmetal_lam.sbdd_env.reinvent4_subprocess_adapter.REINVENT4MultipropertyAdapter`).
        default_components : dict[str, float], optional
            Unused at runtime — kept for API symmetry with the
            config-driven ``from_config`` factory so callers can pass
            either path interchangeably.  The adapter's own
            ``components`` attribute is the source of truth.

        Notes
        -----
        This method does not mutate :attr:`w_reinvent4` — callers
        remain in control of the per-channel weight.  Default is
        ``1.0`` (see :attr:`w_reinvent4`) so a typical call site
        only needs to pass the adapter.
        """
        del default_components  # adapter.components is the source of truth
        if adapter is None or not getattr(adapter, "available", False):
            # Leave r_reinvent4 as None — the aggregator's _safe wrapper
            # already degrades a None channel to 0.0.
            return

        def _channel(state) -> float:
            try:
                # Accept both MoleculeClosedTerm-like objects and plain
                # strings.  We intentionally don't import
                # ``MoleculeClosedTerm`` here to keep this helper usable
                # in test environments that mock it out.
                if isinstance(state, str):
                    smiles = state
                else:
                    fn = getattr(state, "canonical_smiles", None)
                    if callable(fn):
                        ...
                        smiles = str(getattr(state, "smiles", ""))
                if not smiles:
                    return 0.0
                ...
                logger.warning(
                    "RewardAggregator.r_reinvent4: adapter.score() failed (%s); "
                    "falling back to 0.0",
                    exc,
                )
                # Mirror the existing graceful-fail contract — the
                # aggregator already has a _safe wrapper, but a second
                # layer of defence here keeps the closure self-contained
                # so it can be reused outside the aggregator.
                return 0.0
    ...
    def register_reinvent4_api_channel(
        ...
        *,
        ...
        """Wire the ``r_reinvent4`` channel to a :class:`REINVENT4APIAdapter`.

        This is the **opt-in** sibling of
        :meth:`register_reinvent4_multiproperty_channel`.  Where the
        subprocess adapter spawns a ``reinvent4_multiproperty_jsonl_worker``
        child process, the API adapter calls :class:`reinvent.scoring.Scorer`
        directly in the current Python process — provided the user
        has REINVENT4 importable (``pip install -e references/REINVENT4``
        or an equivalent).

        The default ``register_reinvent4_multiproperty_channel`` is
        retained as the **safe** path because the ROCm torch in the
        search venv conflicts with REINVENT4's CUDA torch pin; the API
        path is for users that already have a shared Python
        environment (e.g. Docker / Conda env that ships both).

        Behaviour mirrors the multiproperty channel:

        * When ``adapter.available`` is ``False``, ``r_reinvent4`` is
          left as ``None`` so the aggregator degrades to ``0.0``.
        * When the adapter IS available, the closure extracts the
          SMILES from the molecule state and calls
          ``adapter.score([smiles])[0]`` — the direct-API adapter
          returns the same ``list[float]`` contract.
        * Any exception inside the closure is caught and converted to
          ``0.0`` so the aggregator never crashes (and the adapter's
          ``last_error`` is updated for diagnostics).

        Parameters
        ----------
        adapter : REINVENT4APIAdapter
            The direct-API adapter (see
            :class:`molmetal_lam.sbdd_env.reinvent4_api_adapter.REINVENT4APIAdapter`).
        default_components : dict[str, float], optional
            Unused at runtime — kept for API symmetry with the
            config-driven ``from_config`` factory so callers can pass
            either path interchangeably.  The adapter's own
        ...
        def _channel(state) -> float:
            try:
                ...
                logger.warning(
                    "RewardAggregator.r_reinvent4 (api): adapter.score() "
                    "failed (%s); falling back to 0.0",
                    exc,
                )
                # Mirror the existing graceful-fail contract — the
                # aggregator already has a _safe wrapper, but a second
    ...
    # ------------------------------------------------------------------
    # Phase 2 (Metallodrug ultracode) — PlatinAI predicted-activity oracle
    # ------------------------------------------------------------------
    def register_platinai_oracle_channel(
        self,
        oracle: "Any",
    ) -> None:
        """Wire the ``r_platinai`` channel to a :class:`PlatinAIOracle`.

        Phase 2 of the metallodrug-de-novo ultracode — adds a kNN-based
        predicted-activity reward channel that uses the PlatinAI_MBFinder
        corpus (226,918 SMILES) + two predicted-activity sheets
        (A2780 ovarian, MCF7 breast).  The channel is opt-in via
        ``w_platinai``; the default weight of 0.0 keeps the existing
        reward bit-for-bit identical when unused.

        Behaviour:

        * The supplied ``oracle`` exposes a ``score_channel(state)``
          method (see :class:`molmetal_lam.reward.platinai_oracle.PlatinAIOracle`).
          We install a closure that delegates to that method, mirroring
          the reinvent4 channel contract.
        * The closure accepts both plain SMILES strings and state objects
          (``canonical_smiles()`` / ``smiles`` attribute) — the
          aggregator's ``_safe`` wrapper degrades to 0.0 on any
          exception, so the oracle never crashes MCTS.
        * When ``oracle.available`` is ``False`` (the corpus xlsx files
          were missing or RDKit could not index them), the channel
          silently emits 0.0 for every query — same graceful-degrade
          contract as the REINVENT4 and ADMET channels.

        Parameters
        ----------
        oracle : PlatinAIOracle
            The oracle instance.  Pre-built via
            :class:`molmetal_lam.reward.platinai_oracle.PlatinAIOracle`
            (or the ``make_platinai_channel`` factory).  We keep a
            reference to ``oracle`` so callers can introspect
            ``oracle.n_corpus`` / ``oracle.available`` from outside.

        Notes
        -----
        This method does NOT mutate :attr:`w_platinai` — callers
        remain in control of the per-channel weight.  Default is
        0.0 (off) so existing reward is bit-for-bit unchanged
        when unused; set ``w_platinai = 1.0`` (or any positive
        scalar) to enable the predicted-activity channel.
        """
        self.r_platinai = oracle.score_channel
        self._platinai_oracle = oracle  # keep handle for diagnostics

    # ------------------------------------------------------------------
    # T24 (2026-09-17) — RxnFlow template-match oracle channel (cite-only)
    # ------------------------------------------------------------------
    def register_rxnflow_template_channel(
        self,
        template_name: str = "REAL_001_CuAAC",
    ) -> None:
        """Wire the ``r_rxnflow`` channel to a named RxnFlow template.

        This is the *6th* synthesizability oracle — it answers the
        question "does this candidate look like it could be the OUTPUT
        of a known Enamine REAL reaction template?"  We cite the RxnFlow
        template library (Seo 2024 arXiv:2410.04542) as the closest
        published SMARTS library to our typed-reduction space
        (§\ref{sec:click-chem}) but DO NOT depend on the upstream
        install: the channel is a thin product-side substructure
        match that returns 1.0 if the candidate contains the product
        fragment of the named template, else 0.0.

        Behaviour:

        * The channel is wired to a closure built by
          :func:`molmetal.adapters.rxnflow_adapter.make_rxnflow_template_channel`
          which calls :func:`rxnflow_template_match` on the candidate's
          SMILES.  When RDKit cannot parse the candidate or the template
          SMARTS, the channel returns 0.0 (no crash).
        * When RxnFlow upstream is unavailable (the default in our CI /
          on this 1h36 + 830c host) the channel still operates on the
          product SMARTS — the templates are shipped verbatim in
          ``molmetal/references/RxnFlow/data/templates/real.txt`` and
          the matcher does NOT need the upstream `rxnflow` package
          installed.
        * Default weight is 0.0 (off) so existing reward is bit-for-
          bit identical when unused.

        Parameters
        ----------
        template_name : str, default "REAL_001_CuAAC"
            The template name to match against.  See
            :func:`molmetal.adapters.rxnflow_adapter.list_rxnflow_templates`
            for the full inventory.  Override via the
            ``RXNFLOW_TEMPLATE_NAME`` env-var at runtime.
        """
        # Local import to avoid pulling RDKit at module import time.
        from molmetal.adapters.rxnflow_adapter import (
            make_rxnflow_template_channel,
        )
        self.r_rxnflow = make_rxnflow_template_channel(template_name)
        self._rxnflow_template_name = template_name

    # ------------------------------------------------------------------
    # TODO-30 P2.5 Phase A (2026-09-17) — pharmacophore filter channel
    # ------------------------------------------------------------------
    def register_pharmacophore_channel(
        ...
        *,
        ...
        """Wire the ``r_pharmacophore`` channel to :mod:`pharmacophore_filter`.

        TODO-30 P2.5 Phase A — closes the gap that
        :mod:`molmetal_lam.lam_chem.pharmacophore_filter` shipped as a
        pre-docking boolean gate but was never wired as a continuous
        reward signal in :class:`RewardAggregator`.  This commit adds
        the additive ``r_pharmacophore`` channel; default weight 0.0
        keeps the historical reward bit-for-bit identical when unused.

        The channel returns a continuous pharmacophore score in
        ``[0, 1]`` computed by
        :func:`compute_pharmacophore_score` (a thin wrapper over
        :func:`pass_pharmacophore` that maps ``total_violations`` to a
        partial-credit score ``max(0.0, 1.0 - total_violations/7)``).
        Higher = more drug-like per Lipinski+Veber+ring-count rules.
        The channel gracefully degrades to ``0.0`` on parse failure
        (RDKit cannot parse the candidate SMILES) so the aggregator
        never crashes MCTS.
        ...
        weight : float, default 0.0
            ...
            Defaults to 0.0 so the channel is **opt-in** (backward
            compatible with all 24 REAL adapters that depend on the
            aggregator's bit-for-bit reward surface).  Set 1.0 to make
            a unit-weight contribution; ``>1.0`` over-weights relative
            to the dominant Vina/SA/QED channels.
        strict : bool, default False
            When True, use :func:`pass_pharmacophore` strict mode
            (``total_violations == 0``); the score is then a binary
            0.0/1.0 rather than the partial-credit version.  Useful
            when callers want a hard drug-likeness gate as a reward.
            When False (the default), the lenient mode is used so the
            score is partial-credit.
        allow_acyclic : bool, default False
            Forwarded to :func:`compute_pharmacophore_report`; set
            True when generating metal-acylclic derivatives (cisplatin,
            nedaplatin) where the ring-count rule is bypassed.

        Notes
        -----
        This method does NOT mutate the global :class:`RewardAggregator`
        schema — only the instance fields.  Existing tests that build a
        bare :class:`RewardAggregator()` see no change.  The ``weight``
        parameter is convenience-only (callers can also set
        ``agg.w_pharmacophore = 0.3`` directly).
        """
        # Local import to avoid pulling RDKit at module import time
        # (mirrors the rxnflow pattern at line ~1281 above).
        # The wrapper lives in `pharmacophore_reward_channel.py` so we
        # never have to modify the locked `pharmacophore_filter.py`
        # module — preserves bit-for-bit behaviour for all 24 REAL
        # adapters that depend on the filter as a boolean pre-gate.
        ...
        if strict:
            # Strict mode — return 1.0 if ``total_violations == 0`` else 0.0.
            def _channel(state: Any) -> float:
                try:
                    smi_fn = getattr(state, "canonical_smiles", None)
                    smi = str(smi_fn() if callable(smi_fn) else state)
                except Exception:
                    return 0.0
                try:
                    return 1.0 if pass_pharmacophore(smi, strict=True) else 0.0
                except Exception:
                    return 0.0
        else:
            def _channel(state: Any) -> float:
                ...
                try:
                    return float(
                        compute_pharmacophore_score(
                            smi, allow_acyclic=bool(allow_acyclic)
                        )
                    )
                except Exception:
                    return 0.0

        self.r_pharmacophore = _channel
        self.w_pharmacophore = float(weight)
        self._pharmacophore_strict = bool(strict)
        self._pharmacophore_allow_acyclic = bool(allow_acyclic)

    # ------------------------------------------------------------------
    # TODO-30 P5.2 Tier 1 (2026-09-17) — wet-lab feedback channel
    # ------------------------------------------------------------------
    def register_wetlab_channel(
        self,
        measurement_dict: "Optional[Any]" = None,
        *,
        weight: float = 0.0,
        predicted_lookup: "Optional[Any]" = None,
        zero_when_missing: bool = True,
    ) -> None:
        """Wire the ``r_wetlab`` channel to :mod:`wetlab_reward_channel`.

        TODO-30 P5.2 Tier 1 — adds the wet-lab signed-error channel
        ``-|predicted - measured|`` to :class:`RewardAggregator`.
        Default ``w_wetlab=0.0`` keeps the historical reward
        bit-for-bit identical when unused.

        The channel degrades to 0.0 on every failure mode (missing
        measurement, NaN, RDKit parse failure) so it never crashes
        MCTS.  When ``measurement_dict`` is ``None`` the closure is a
        permanent no-op (still safe to install).

        Parameters
        ----------
        measurement_dict : Mapping[str, float] | None
            ``{smiles: pIC50}`` map.  Typically produced by
            :func:`molmetal_lam.lam_chem.wetlab_protocol.assays_to_dict`
            on the output of :func:`molmetal_lam.lam_chem.wetlab_protocol.load_assays`.
            When ``None`` the closure is a no-op (returns 0.0 for
            every candidate).
        weight : float, default 0.0
            Forwarded to :attr:`w_wetlab`.  Defaults to 0.0 so the
            channel is opt-in (backward compatible with all 24 REAL
            adapters).  Set 1.0 for a unit-weight contribution.
        predicted_lookup : Callable[[str], Optional[float]], optional
            Forwarded to the channel closure.  When ``None`` the
            channel silently degrades to ``-|measured|`` which is
            still well-bounded but slightly penalises any measured
            candidate.  Production callers should wire the project
            pIC50 predictor here.
        ...
        Notes
        -----
        This method does NOT mutate :attr:`w_wetlab` — callers remain
        in control of the per-channel weight (mirrors the PlatinAI
        contract).  Default 0.0 (off) keeps existing reward
        bit-for-bit unchanged when unused.
        """
        # Local import mirrors the rxnflow + pharmacophore pattern so
        # the channel does not pull RDKit at module import time.
        from molmetal_lam.lam_chem.wetlab_reward_channel import (
            make_wetlab_channel,
        )

        self.r_wetlab = make_wetlab_channel(
            measurement_dict,
            predicted_lookup=predicted_lookup,
            zero_when_missing=bool(zero_when_missing),
        )
        self.w_wetlab = float(weight)
        try:
            self._wetlab_n_loaded = (
                int(len(measurement_dict)) if measurement_dict else 0
            )
        except Exception:
            self._wetlab_n_loaded = 0

    # ------------------------------------------------------------------
    # F5 learned shaping (WF-Deflex Wire-up Phase 1) — opt-in controller
    # ------------------------------------------------------------------
    def register_learned_shaping_channel(
        self,
        *,
        enabled: bool = True,
        formula: "Optional[Any]" = None,
    ) -> None:
        """Wire the F5 symbolic-reward shaping channel.

        WF-Deflex Wire-up Phase 1 — closes the "F5 ships but is not
        wired into the production reward" gap.  When
        ``enabled=True`` (the default), :meth:`__call__` will replace
        the SA-prior contribution with ``R_F5(sa_score)`` scaled by
        :attr:`w_learned_shaping`.  When ``enabled=False`` the channel
        is a no-op even if :attr:`use_learned_shaping` is True.

        The channel reads the **raw** SA score (1..10) before the
        inversion in :meth:`__call__`, so the F5 formula sees the
        canonical SA scale.  The result is added to the aggregate on
        top of the existing channels so a call to
        ``register_learned_shaping_channel(enabled=True)`` is the
        bit-for-bit unique change.

        Parameters
        ----------
        enabled : bool, default True
            When False, :attr:`use_learned_shaping` is forced off
            regardless of the env-var or flag, and the F5 contribution
            is silently 0.0.  When True, the gate is the OR of
            ``enabled``, :attr:`use_learned_shaping`, and the
            ``LEARNED_SHAPING_ENABLED`` env-var — any one being truthy
            flips the channel on.
        formula : BestFormula, optional
            Override the default F5 formula (see
            :func:`molmetal_lam.reward.learned_shaping.load_best_formula`).
            Useful for callers that have already re-fit on a new cohort
            via
            :func:`molmetal_lam.reward.learned_shaping.fit_from_metrics`.

        Notes
        -----
        The learned shaping is computed at every MCTS leaf; the F5
        formula is O(1) per evaluation (a single multiply-add on the
        SA scalar), so the wire-up adds negligible overhead.

        Honest framing
        --------------
        The F5 formula was recovered from n=33 MEASURED Round-12 path-(a)
        cells with LassoLarsIC+BIC.  In-sample R^2 = 1.0 (BIC picked
        exact sparsity); LOO R^2 = -0.0635 — close to random.  The
        wire-up is INTERPRETIVE (it identifies sa_norm as the lone
        surviving signal), not predictive.  Production pilots should
        log the F5 contribution alongside the dominant channels and
        NOT trust the F5 lift as a measurement without re-fit
        validation on the target cohort.
        """
        if formula is not None:
            # Caller supplied a custom formula — install it on the
            # internal LearnedShaping singleton.  We don't keep a
            # long-lived handle because the formula is queried
            # lazily via load_best_formula() the first time the
            # channel fires.
            try:
                from molmetal_lam.reward import learned_shaping as _ls
                # Swap the module-level default for this call-site.
                # We keep the env-var gate intact so the same
                # LearnedShaping instance is reused across calls.
                _ls_mod = _ls
                # Stash on self so __call__ can override per-aggregator.
                self._learned_shaping_formula_override = formula
            except Exception:
                # Module unavailable — fall back to the hardcoded
                # F5 formula via load_best_formula() at call time.
                self._learned_shaping_formula_override = None
        else:
            self._learned_shaping_formula_override = None
        self.use_learned_shaping = bool(enabled)

    def _learned_shaping_active(self) -> bool:
        """Return True iff the F5 channel fires at :meth:`__call__` time.

        Reads ``LEARNED_SHAPING_ENABLED`` lazily so callers can flip
        the env-var between MCTS iterations without rebuilding the
        aggregator.
        """
        try:
            if bool(self.use_learned_shaping):
                return True
        except Exception:
            pass
        try:
            v = os.environ.get(self._LEARNED_SHAPING_ENV, "0").strip().lower()
            return v in ("1", "true", "yes", "on")
        except Exception:
            return False

    def _learned_shaping_contribution(self, sa_raw: float) -> float:
        """Return the F5 contribution for the current SA scalar.

        Returns ``0.0`` when the channel is inactive.  When active,
        returns ``w_learned_shaping * R_F5(sa_raw)``.  Lazy import of
        :mod:`molmetal_lam.reward.learned_shaping` keeps the
        production import surface lean when F5 is unused.
        """
        if not self._learned_shaping_active():
            return 0.0
        try:
            ...
            formula = getattr(
                self, "_learned_shaping_formula_override", None
            ) or load_best_formula()
            sh = LearnedShaping(formula=formula)
            sh.enable()
            return float(self.w_learned_shaping) * float(sh.shape_reward(sa_raw))
        except Exception as exc:
            # Graceful degradation — never crash the aggregator on a
            # F5 import / evaluation error.  Logs the failure but
            # returns 0.0 so the dominant channels still fire.
            try:
                logger.warning(
                    "RewardAggregator._learned_shaping_contribution: "
                    "F5 evaluation failed (%s); falling back to 0.0",
                    exc,
                )
            except Exception:
                pass
    ...
    def aggregate(
        ...
        * ``channels`` is an optional ``{channel_name: value}`` mapping.
        ...
        if not channels:
            try:
                w_total = (
                    ...
                    + float(self.w_synth)
                    + float(self.w_admet)
                    + float(self.w_platinai)
                    + float(self.w_lambda_score)
                    + float(self.w_wetlab)
                )
            except Exception:
                w_total = 0.0
        ...
        weight_for_channel = {
            ...
            "r_synth": "w_synth",
            "r_admet": "w_admet",
            "r_anticancer_index": "w_anticancer_index",
            "r_platinai": "w_platinai",
            "r_lambda_score": "w_lambda_score",
            # TODO-30 P2.5 Phase A — pharmacophore reward channel.
            "r_pharmacophore": "w_pharmacophore",
            # TODO-30 P5.2 Tier 1 — wet-lab signed-error channel.
            "r_wetlab": "w_wetlab",
        }
        value = 0.0
    ...
    def __call__(
        ...
        *,
        ...
        # Phase 3H — soft, continuous metal-geometry channel.  Opt-in
        # via ``w_metal_geom_soft > 0``; gracefully degrades to 0.0
        # when the channel is None or when the molecule has no metal
        # centre (organic ligands don't earn a metal bonus).
        v_metal_geom_soft = _safe(self.r_metal_geom_soft)
        # TODO-15 — equal-weight anticancer_index channel (opt-in via
        # ``w_anticancer_index > 0``; defaults to 0.0 so existing
        # weighted reward stays bit-for-bit identical when unused).
        v_anticancer_index = _safe(self.r_anticancer_index)
        # Phase 2 (Metallodrug ultracode) — PlatinAI predicted-activity
        # oracle channel (opt-in via ``w_platinai > 0``; defaults to
        # 0.0 so existing weighted reward stays bit-for-bit identical
        # when unused).  The oracle returns values in [0, 1] so the
        # scalar weight maps directly to the aggregated reward.
        v_platinai = _safe(self.r_platinai)
        # T24 — RxnFlow template-match oracle channel (cite-only
        # fallback).  Opt-in via ``w_rxnflow > 0``; defaults to 0.0 so
        # existing weighted reward is bit-for-bit unchanged when unused.
        # Returns 1.0 if the candidate matches the named template's
        # product SMARTS, else 0.0 (RDKit substructure match).  Falls
        # back to 0.0 when the upstream is missing or RDKit fails to
        # parse — never crashes MCTS.
        v_rxnflow = _safe(self.r_rxnflow)
        # TODO-30 P2.5 Phase A (2026-09-17) — pharmacophore continuous
        # reward channel.  Returns a value in [0, 1] (1.0 = strict
        # Lipinski+Veber+ring-count pass; 0.0 = all violations).
        # Default impl wraps
        # :func:`molmetal_lam.lam_chem.pharmacophore_reward_channel.compute_pharmacophore_score`
        # which is a thin wrapper over the locked
        # :mod:`pharmacophore_filter` boolean gate.  Opt-in via
        # ``w_pharmacophore > 0``; default 0.0 so existing reward is
        # bit-for-bit unchanged when unused.  Falls back to 0.0 on
        # RDKit parse failure or any internal exception — never
        # crashes MCTS.
        v_pharmacophore = _safe(self.r_pharmacophore)
        # TODO-21 Strategy 1 (2026-09-17) — Lambda-as-reward channel.
        # Reads the module-level ``_R_LAMBDA_CANDIDATES`` global (set by
        # the harness via :func:`set_lambda_candidates`) and computes the
        # mean cosine similarity to the current CFM pocket embedding.
        # Opt-in via ``w_lambda_score > 0``; default ``0.0`` so the
        # existing weighted reward is bit-for-bit unchanged when unused.
        # The closure is installed via
        # :func:`molmetal_lam.lam_chem.lambda_reward_channel.register_lambda_reward_channel`
        # and gracefully degrades to 0.0 on every failure mode (empty
        # candidates, coupling disabled, NaN, exception).
        r_lambda_score_callable = getattr(self, "r_lambda_score", None)
        w_lambda_score_attr = float(getattr(self, "w_lambda_score", 0.0))
        v_lambda_score = _safe(r_lambda_score_callable) if r_lambda_score_callable else 0.0
        ...
        value += self.w_anticancer_index * v_anticancer_index
        # Phase 3H — soft, continuous metal-geometry channel.  Contributes
        # a non-zero reward across the candidate spectrum so MCTS does
        # not collapse onto the metal seed (cf. Schulman 2017 ratio
        # clipping and Neu 2017 entropy-regularised MDPs).
        value += self.w_metal_geom_soft * v_metal_geom_soft
        # Phase 2 (Metallodrug ultracode) — PlatinAI predicted-activity
        # oracle channel.  Default weight is 0.0 so existing reward is
        # bit-for-bit unchanged when the channel is not wired.
        value += self.w_platinai * v_platinai
        # T24 — RxnFlow template-match oracle channel.  Default weight
        # 0.0 (off) so existing reward is bit-for-bit unchanged when
        # unused; set ``w_rxnflow = 1.0`` to make the template-match
        # contribute on the same footing as the other reward channels.
        value += self.w_rxnflow * v_rxnflow
        # TODO-30 P2.5 Phase A — additive pharmacophore reward channel.
        # Default weight 0.0 (off) so existing reward is bit-for-bit
        # unchanged when unused; set ``w_pharmacophore = 1.0`` to make
        # the continuous Lipinski+Veber+ring score contribute on the
        # same footing as the other reward channels.  Because the
        # channel returns ``1 - violations/7`` (range [0, 1]), the
        # additive contribution is bounded by ``w_pharmacophore``
        # (i.e. at most ``+1.0`` per leaf when weight=1.0).
        value += self.w_pharmacophore * v_pharmacophore
        # TODO-30 P5.2 Tier 1 — wet-lab signed-error channel.  Returns
        # ``-|predicted - measured|`` for assayed candidates (range
        # [-3, 0] clamped); ``0.0`` otherwise.  Default weight 0.0
        # (off) so existing reward is bit-for-bit unchanged when
        # unused; set ``w_wetlab = 1.0`` to make the signed-error term
        # contribute on the same footing as the other reward
        # channels.  Opt-in — even when the closure is installed via
        # :meth:`register_wetlab_channel`, the weight controls
        # whether the contribution is summed into the aggregate.
        v_wetlab = _safe(self.r_wetlab)
        value += self.w_wetlab * v_wetlab
        # TODO-21 Strategy 1 — additive Lambda-as-reward channel.  Only
        # fires when ``w_lambda_score > 0``; default 0.0 so the existing
        # reward sum is bit-for-bit unchanged.
        value += w_lambda_score_attr * v_lambda_score
        # WF-Deflex Wire-up Phase 1 — F5 symbolic-reward shaping.  When
        # the channel is active (via ``use_learned_shaping`` flag or
        # ``LEARNED_SHAPING_ENABLED=1`` env-var) we apply the F5
        # formula ``2.5836 - 2.5149 * sa_raw`` and add it as an
        # *additive* contribution on top of the existing reward.  This
        # is the wire-up that closes the "F5 ships but is not wired"
        # gap documented in wf_deflex_followup §phase1.  When inactive
        # the contribution is exactly 0.0 so existing reward is
        # bit-for-bit unchanged.  Lit anchor: McAllester 1999 PAC-Bayes
        # regularised linear (sparse LassoLarsIC fit on n=33 MEASURED
        # cells picked sa_norm as the lone surviving signal).
    ...
    def metrics(self):
        ...
        return {k: v for k, v in {
            ...
            "anticancer_index": self.r_anticancer_index,
            "platinai": self.r_platinai,
...
#: SMARTS-defined click rules whose underlying reaction SMARTS is written
#: with a fixed reactant order (azide / boronic-acid first).  For these
#: rules, :meth:`MCTSProofSearch._safe_reduce` retries ``rule.reduce``
#: with swapped arguments when the forward call returns no products.
#: See :func:`molmetal_lam.reactions.beta_reductions._run_reactants_symmetric`
#: for the lower-level dispatch helper.
#:
#: This list is consulted by ``_safe_reduce`` only — adding a rule here
#: is a one-line change and does NOT require touching the rule's own
#: ``_reduce`` method (although we also patched CuAAC/SPAAC/Suzuki in
#: place for symmetry when called via the standalone ``reduce()`` API).
_SYMMETRIC_RULE_NAMES = frozenset({"CuAAC", "SPAAC", "Suzuki"})
...
def _pareto_score_vector(
    state: MoleculeClosedTerm,
    score: float,
    reward_fn: Optional["RewardAggregator"] = None,
) -> List[float]:
    """Compose the multi-objective score vector used by the Pareto ranker.

    TODO-30 / Rank-1 P3.3 — extracts the per-channel reward vector
    that :func:`molmetal_lam.search_alg.pareto.rank_population` will
    operate on.  By default we use a 4-D vector:

      [scalar_reward, 1 - sa, qed, vina_proxy_inverted]

    so the Pareto front breaks ties between molecules with identical
    scalar reward by per-channel dominance (drug-likeness, synthetic
    accessibility, predicted affinity).  Lit anchor: Deb 2002 NSGA-II
    uses a per-objective vector of comparable length.

    When ``reward_fn`` is ``None`` or any sub-channel raises, the
    channel degrades gracefully to ``0.0`` so the vector is well-defined
    even in headless test runs (where RDKit or the score aggregator
    may be unavailable).

    Parameters
    ----------
    state : MoleculeClosedTerm
        Candidate molecule state.  Only used as a handle to invoke the
        reward channels; not serialised.
    score : float
        The scalar aggregated reward already on the candidate
        (precomputed by ``score_final`` in :meth:`MCTSProofSearch.search`).
        Used as the first objective to keep the ranker monotone in the
        scalar reward — degenerate inputs collapse to the legacy order.
    reward_fn : RewardAggregator, optional
        The aggregator carrying the channel callables.  When ``None``
        we fall back to the constant-zero vector ``[score, 0, 0, 0]``
        so the Pareto ranker still orders by scalar reward (and
        crowding-distance inside a rank).

    Returns
    -------
    list[float]
        Length-4 vector of maximised objectives.  All objectives are
        MAXIMISED by construction — Vina is negated upstream in
        :meth:`RewardAggregator.__call__` so the legacy ``r_vina``
        channel returns *positive* values for *better* docking.
    """
    out: List[float] = [float(score), 0.0, 0.0, 0.0]
    ...
    try:
        # SA raw is 1 (easy) .. 10 (hard); invert so higher = easier.
        sa_raw = _safe(reward_fn.r_sa) if reward_fn.r_sa is not None else 0.0
        out[1] = (
            max(0.0, min(1.0, 1.0 - (sa_raw - 1.0) / 9.0))
            if reward_fn.r_sa
            else 0.0
        )
        # QED is already in [0, 1]; higher = more drug-like.
        out[2] = _safe(reward_fn.r_qed)
        # Vina proxy is already [0, 1] with higher = better after
        # the legacy 1 - sa/10 fallback in
        # :meth:`RewardAggregator.compute_batch_rdkit` or the explicit
        # :func:`_default_vina_proxy_channel`.
        out[3] = _safe(reward_fn.r_vina_proxy)
    except Exception:
        # Final defensive — never let the ranker crash the search.
        return [float(score), 0.0, 0.0, 0.0]
    return out


def _pareto_rank_candidates(
    candidates: List[Tuple[float, MoleculeClosedTerm]],
    reward_fn: Optional["RewardAggregator"] = None,
    weights: Optional[Sequence[float]] = None,
) -> List[Tuple[float, MoleculeClosedTerm]]:
    """Sort ``candidates`` by NSGA-II Pareto rank + crowding distance.

    TODO-30 / Rank-1 P3.3 — additive wire-in of the previously
    unused :mod:`molmetal_lam.search_alg.pareto` ranker.  This
    helper is the **only** place where the existing legacy sort

        candidates.sort(key=lambda p: p[0], reverse=True)

    is augmented; the ranker is opt-in via ``MCTSProofSearch.pareto_rank_top_k``
    so the legacy scalar-reward ordering is preserved bit-for-bit when
    the flag is OFF (the default).

    Behaviour:

    * Compose the multi-objective vector for every candidate via
      :func:`_pareto_score_vector`.
    * Delegate to :func:`pareto.rank_population` which applies the
      iterated non-domination sort (Deb 2002 §III.A) and crowding-
      distance comparison (Deb 2002 §III.B).
    * Re-order the input list in place — the caller's reference is
      mutated so the surrounding ``search()`` flow is unchanged.

    Fallback: if the Pareto module fails to import or the ranker raises
    for ANY reason (NumPy shape mismatch, NaN inputs, etc.), we log
    and return the legacy ``sorted(..., key=lambda p: p[0], reverse=True)``
    order so the wire-in is **strictly additive** — the OFF path
    remains bit-for-bit identical, and the ON path degrades gracefully
    to the legacy order on failure rather than crashing MCTS.

    Parameters
    ----------
    candidates : list[tuple[float, MoleculeClosedTerm]]
        The candidate set, in legacy (score, state) form, as produced
        by the search loop.
    reward_fn : RewardAggregator, optional
        The aggregator carrying the channel callables used by
        :func:`_pareto_score_vector`.
    weights : Sequence[float], optional
        Forwarded to :func:`pareto.rank_population` as the weighted-
        sum tie-breaker.  When ``None`` the ranker uses equal weights.

    Returns
    -------
    list[tuple[float, MoleculeClosedTerm]]
        The same list object, re-ordered in place.  The legacy
        scalar-reward ordering is preserved as the secondary key.
    """
    if not candidates:
        return candidates
    try:
        from molmetal_lam.search_alg import pareto as _pareto  # late import
        pop: List[List[float]] = [
            _pareto_score_vector(state, score, reward_fn=reward_fn)
            for score, state in candidates
        ]
        order = _pareto.rank_population(
            pop, weights=list(weights) if weights is not None else None
        )
        reordered = [candidates[i] for i in order]
        # Mutate in place so the surrounding search() flow is unaffected.
        candidates[:] = reordered
        return candidates
    except Exception as exc:  # pragma: no cover — defensive
        try:
            log.debug(
                "_pareto_rank_candidates: ranker failed (%s); "
                "falling back to legacy scalar-reward order",
                exc,
            )
        except Exception:
            pass
...
class MCTSProofSearch:
    ...
    #: TODO-30 P1.2 (2026-09-17) — opt-in FG-compatibility veto at the
    #: MCTS layer.  When False (default) every ReactionRule.reduce()
    #: call is the legacy *permissive* path — backward-compatible with
    #: Round-12/13 measurements.  When True every reduction pass through
    #: :meth:`_safe_reduce` consults
    #: :func:`molmetal_lam.reactions.fg_compatibility.vet_smiles_against_rule`
    #: and rejects (returns ``[]``) any reactant whose SMILES has a
    #: disfavoured FG and no tolerated FG.  CLI flag:
    #: ``--fg-veto-strict`` in
    #: :file:`molmetal/scripts/r4_lambda_only_run.py`.  Property-test
    #: gated: the veto fires ONLY when (a) ``fg_veto_strict=True`` AND
    #: (b) reactant SMILES has a disfavoured FG AND (c) reactant SMILES
    #: is missing a tolerated FG.  Negative-false-positive rate = 0 in
    #: :file:`tests/test_fg_compatibility.py`.
    fg_veto_strict: bool = False

    #: TODO-30 / Rank-1 P3.3 — Pareto-rank postprocess at end of search.
    #: When False (default) the candidate ordering is the legacy
    #: ``sorted(..., key=lambda p: p[0], reverse=True)`` — bit-for-bit
    #: backward compatible.  When True the candidate set is re-ranked
    #: by NSGA-II Pareto-rank + crowding-distance (Deb 2002) using
    #: the multi-objective score vector assembled by
    #: :func:`_pareto_score_vector`.  Lit anchors: Deb 2002 IEEE TEVC
    #: 6(2):182-197 (NSGA-II), Zitzler & Thiele 1999 IEEE TEVC
    #: 3(4):257-271 (SPEA / multi-objective baseline).  Opt-in to keep
    #: existing scalar-reward ablations bit-for-bit identical when
    #: unused.  CLI flag: ``--postprocess-pareto`` in
    #: :file:`molmetal/scripts/r4_lambda_only_run.py`.
    pareto_rank_top_k: bool = False
    #: TODO-30 / Rank-1 P3.3 — Optional scalar weight vector applied as
    #: a tie-breaker inside :func:`pareto.rank_population` so the
    #: weighted-sum and Pareto fronts agree on the canonical
    #: scalar-collapse fallback (Zitzler 1999 §3).  When ``None`` we
    #: use the equal-weight vector ``[1.0] * n_objectives``.
    pareto_weights: Optional[Sequence[float]] = None
    ...
    def search(
        ...
        *,
        ...
        use_pocket_macro: bool = False,
        pocket_macro_target_name: Optional[str] = None,
        use_learned_prior_argmax: bool = False,
        ...
        materialize_3d : bool, default True
            TODO-30 / Rank-7 P2.2 — default flipped False→True on
            2026-09-17 so every emitted candidate carries a
            ``coords_3d`` attribute by default (the
            WF-Lambda-MCTS-Coords-Fix co-emit contract).  When True,
            every emitted candidate has a ``coords_3d`` attribute
            attached to it.  ``coords_3d`` is a ``torch.Tensor`` of
            shape ``(n_heavy_atoms, 3)`` carrying the ETKDGv3 3-D
            conformer of the molecule.  This is a *side-effect* on
            the candidate object — the canonical SMILES is unchanged
            so existing callers that only inspect SMILES are not
            affected.  When False (escape hatch for pure 2D
            generation — e.g. ``r4_lambda_only_run.py
            --no-materialize-3d``), no attribute is attached.
        pocket_features : PocketFeatureVector, optional
            WF-Pocket-Invariance Phase-3J — when non-None, the root
            prior ``P`` is set via
            :func:`molmetal_lam.search_alg.warm_start.modify_root_prior`
            using ``pocket_features_vec=pocket_features``.  When
            ``None`` (default — backward-compatible), the root
            prior is the legacy constant-0.5 stub and the search
            produces *pocket-invariant* candidate lists (TODO-29).
        learned_prior : LearnedPolicyPrior, optional
            WF-Pocket-Invariance Phase-3L — when non-None, every
            :meth:`_prior` evaluation mixes the learned per-state
            prior with a uniform prior (AGZ root-noise convention).
            When ``None`` (default — backward-compatible), the prior
            stays constant-0.5 and the search is *learned-blind*.
        learned_prior_mix_uniform : float, default 0.5
            Mixing weight ``α = 1 - mix_uniform`` on the *learned*
            leg of the AGZ-style prior blend.  0.0 = pure learned
            (no uniform leg), 1.0 = pure uniform (no learned
            contribution).  Default 0.5 is the AGZ root-noise
            convention.

        Notes
        -----
        * ``materialize_3d`` is **best-effort** — a SMILES that fails
          RDKit's ``EmbedMolecule`` (e.g. very unusual coordination
          motifs) falls back to leaving ``coords_3d`` unset, so
          callers must check ``hasattr(cand, 'coords_3d')`` before
          using the tensor.  Failures are recorded on
          ``self.history[-1]['n_coords_3d_attached']`` for audit.
        * This is the *co-emit* pathway requested by
          WF-Lambda-MCTS-Coords-Fix: callers that want a separate
          SMILES → coords mapping can iterate ``search()`` and run
          RDKit directly, but for tight integration
          (r4_lambda_only_run.py pipeline) we attach the coords here
          so the downstream scaffold-aware gate + ReworkedDecoder
          can operate on the same candidate object.
        * No change to the canonical SMILES or atom connectivity —
          the coord tensor is an extra attribute only.
        * The new ``pocket_features`` and ``learned_prior`` kwargs
          are keyword-only (after ``*``), so the positional call
          shape is unchanged.  When both are ``None``, the search
          is **bit-for-bit identical** to the legacy
          ``search(initial_state, max_depth, materialize_3d=False)``.
        use_sa_prior : bool, default False
            WF-T25-SA-Prior — when ``True``, the MCTS leaf prior is
            augmented with an SA-aware logit
            ``prior_logit(x) = -log(sa(x))`` so lower-SA candidates
            (more drug-like / easier to synthesise) are biased
            during PUCT expansion.  Falls back to a uniform 0.0
            logit (= the legacy 0.5 constant prior) when the
            SMILES is unparseable or no SA backend is available.
            Backward-compatible: when ``False`` (default), the
            search is bit-for-bit identical to the prior version.
            Distinct from ``--sa-weight`` (post-hoc reward channel):
            ``--use-sa-prior`` biases the *expansion policy*, not the
            leaf value.
        sa_prior_strength : float, default 0.5
            Mixing weight ``β = sa_prior_strength`` between the
            existing prior mass (returned by :meth:`_prior`) and the
            SA-derived prior mass.  0.0 = pure legacy, 1.0 = pure
            SA-aware.  Default 0.5 mirrors the AlphaGo-Zero root-
            noise convention.  Only consulted when
            ``use_sa_prior=True``.
        """
        if self.rng is None:
            self.rng = random.Random()

        # WF-Deflex Wire-up Phase 2 — PocketMacroInference adapter.
        # When ``use_pocket_macro=True`` AND ``pocket_features`` is
        # None (the default), lazily call PocketMacroInference to
        # build a 32-d pocket-conditioned feature vector from the
        # v2 checkpoint.  The vector is then forwarded to
        # ``modify_root_prior`` exactly like the hand-crafted
        # hash-projection ``pocket_features`` path.  When the
        # gate is off (default) or the target_name is None, the
        # feature vector is ``None`` and the existing branch falls
        # back to the legacy 0.5 constant prior — bit-for-bit identical
        # behaviour.
        if (
            use_pocket_macro
            and pocket_features is None
            and pocket_macro_target_name is not None
        ):
            try:
                from molmetal_lam.lam_chem.pocket_macro_inference import (
                    PocketMacroInference,
                )
                _pmi = PocketMacroInference()
                if _pmi.is_available():
                    pocket_features = _pmi.get_embedding(
                        pocket_macro_target_name
                    )
                else:
                    # TODO-30 / Rank-7 P2.2 (2026-09-17) — pocket
                    # embedding skipped because PocketMacroInference
                    # is not available (no checkpoint loaded).  Log a
                    # DEBUG-level message so operators can verify the
                    # skip is intentional without flooding the logs.
                    # Falls back to the legacy 0.5 root prior.
                    pocket_features = None
                    try:
                        import logging as _logging_p22  # type: ignore
                        _logging_p22.getLogger(__name__).debug(
                            "pocket_macro_inference not loaded; "
                            "skipping pocket embedding for "
                            "pocket_macro_target_name=%r "
                            "(TODO-30 P2.2 guard).",
                            pocket_macro_target_name,
                        )
                    except Exception:
                ...
                # Sticky fallback — never fail search() on a
                # pocket-macro import / inference error.  Falls back
                # to the legacy 0.5 root prior.
                pocket_features = None

        reward_fn = self._resolved_reward()

        # WF-Pocket-Invariance Phase-3L — stash the learned-prior hooks
        # on the search instance so :meth:`self._prior` (called at the
        # root init below, and again on every child expansion) can
        # consult them.  Reset to None at the top of every ``search()``
        # call so a previous run's hooks do not leak.
        self._learned_prior_for_search = learned_prior
        self._learned_prior_mix_uniform_for_search = float(
            learned_prior_mix_uniform
        )

        # WF-T25-SA-Prior — stash the SA-aware prior hooks on the
        # search instance so :meth:`self._prior` can consult them on
        # every child expansion.  When ``use_sa_prior=False`` (the
        # default) the legacy behaviour is bit-for-bit preserved.
        # When ``use_sa_prior=True`` we lazily construct an
        # :class:`SAPrior` instance on the first ``_prior`` call.
        self._use_sa_prior_for_search = bool(use_sa_prior)
        self._sa_prior_strength_for_search = float(sa_prior_strength)
        self._sa_prior_obj_for_search = None

        # WF-Pocket-Invariance Phase-3 (sub-fix C) — when the caller
        # supplies ``pocket_features`` AND ``use_pocket_conditioned_reference
        # =True``, swap the MCTS root state for a pocket-conditioned
        # reference ligand SMILES returned by
        # :func:`molmetal_lam.lam_chem.reference_ligand_resolver.resolve_reference_ligand`.
        # The 5-pocket lookup table maps pocket_name → distinct SMILES
        # (CA2 = sulfonamide-arene, MMP2 = hydroxamate-peptide,
        # HDAC2 = thiol-coumarin, pocket_007 = quinoline-alkyne,
        # pocket_011 = benzamide-imide).  Each pocket thus starts the
        # MCTS from a *different* initial state — breaking the lambda-
        # calculus-level cache collapse (sub-fix C complements the
        # metal-level sub-fix C' which maps pocket → metal atom).
        # When ``pocket_features is None`` the resolver falls back to
        # the legacy cisplatin seed ``[Pt]C#C`` and emits a WARN, so
        # callers MUST supply ``pocket_features`` for sub-fix C to fire.
        # When ``use_pocket_conditioned_reference=False`` (default) the
        # resolver is NOT consulted and the search uses the caller's
        # ``initial_state`` verbatim — bit-for-bit backward compatible.
        if (
            use_pocket_conditioned_reference
            and pocket_features is not None
        ):
            try:
                from molmetal_lam.lam_chem.reference_ligand_resolver import (
                    resolve_reference_ligand,
                )
                _ref_record = resolve_reference_ligand(
                    pocket_features,
                    warn_on_fallback=False,
                )
                try:
                    initial_state = MoleculeClosedTerm.from_smiles(
                        _ref_record.smiles,
                        embed_3d=False,
                    )
                    self._reference_ligand_record_for_search = (
                        _ref_record
                    )
                except Exception:
                    # Sticky fallback — if the resolver SMILES fails
                    # to round-trip via MoleculeClosedTerm.from_smiles
                    # the legacy initial_state stands.  We still stash
                    # the record so downstream diagnostics see the
                    # resolver's intent.
                    self._reference_ligand_record_for_search = (
                        _ref_record
                    )
                ...
                # Sticky fallback — never fail search() on a
                # resolver import / call error.  Legacy initial_state
                # stands.
                self._reference_ligand_record_for_search = None
        ...
        # WF-Pocket-Invariance Phase-3J — when ``pocket_features`` is
        # supplied, override the root prior with a pocket-conditioned
        # distribution from
        # :func:`molmetal_lam.search_alg.warm_start.modify_root_prior`.
        # This breaks the "novel pockets produce identical candidate
        # lists" failure mode documented in TODO-29 (w9u99n4jh smoke).
        # When ``pocket_features`` is None (the default — backward-
        # compatible), the root prior is the legacy
        # :meth:`self._prior` value bit-for-bit.
        if pocket_features is not None:
            try:
                from molmetal_lam.search_alg.warm_start import (
                    modify_root_prior,
                )
                # Enumerate the root's children via the existing
                # _expand helper (mirror the first call to get the
                # action keys so modify_root_prior gets the correct
                # shape).
                root_actions: List[Any] = []
                try:
                    expand_cache = self._expand(initial_state)
                    for entry in expand_cache:
                        if isinstance(entry, tuple) and len(entry) >= 3:
                            prod, rule_name, tile = entry[0], entry[1], entry[2]
                            try:
                                tile_smi = (
                                    str(tile.canonical_smiles() or "")
                                    if tile is not None
                                    else ""
                                )
                            except Exception:
                            ...
                            root_actions.append(str(entry))
                except Exception:
                    expand_cache = []
                    root_actions = []
                if root_actions:
                    try:
                        state_feats_seq: List[float] = []
                        try:
                            phi_state = self.heuristic(initial_state)
                        ...
                        pocket_prior = modify_root_prior(
                            root_state_features=state_feats_seq,
                            pocket_features_vec=pocket_features,
                            actions=root_actions,
                        )
                        # Apply the pocket-conditioned prior to the
                        # root's children.  We use the argmax boost
                        # strategy: lift the matching child to the
                        # pocket score in [0.5, 1.0], leave the rest
                        # at the legacy 0.5.  This is a minimal
                        # perturbation — the per-child P stays a
                        # scalar so the dataclass invariant holds.
                        if pocket_prior:
                            # WF-Pocket-Invariance Phase-2A (sub-fix A)
                            # critical fix: the w2swi9tsu Phase-3 patch
                            # had ``if pocket_prior and root.children``
                            # but ``root.children`` is EMPTY at this
                            # point (root was just created on line 2425
                            # with no children attached).  The boost
                            # loop was therefore a no-op.  Fix: call
                            # ``_attach_children`` BEFORE the boost loop
                            # so root.children exists, and stash
                            # expand_cache on ``self`` so ``_simulate``
                            # reuses the inflated children instead of
                            # creating fresh nodes with P=0.5.
                            argmax_action = max(
                                pocket_prior, key=pocket_prior.get
                            )
                            # WF-Pocket-Invariance Phase-2A (sub-fix A):
                            # the w2swi9tsu Phase-3 patch clamped the
                            # pocket-argmax child's P to [0.5, 1.0],
                            # which discards magnitude information and
                            # forces a uniform 0.5 on every non-argmax
                            # child — the pocket signal could not
                            # propagate when the rank order of pocket
                            # scores was preserved (see phase1_diagnose
                            # §4.4).  Lower the floor from 0.5 -> 0.05
                            # so the pocket signal can propagate even
                            # when modify_root_prior returns a soft
                            # distribution, and add
                            # ``pocket_boost_strength`` so callers can
                            # inflate the argmax above the legacy 0.5
                            # baseline.  When ``pocket_boost_strength
                            # > 1.0``, allow up to 99% of mass on the
                            # pocket-argmax child (capped at 1.0 to
                            # preserve the dataclass invariant
                            # ``P in [0, 1]``).
                            base_argmax_prob = float(
                                pocket_prior[argmax_action]
                            )
                            strength = float(pocket_boost_strength)
                            if strength > 1.0:
                                # Stronger boost: inflate the argmax
                                # mass by ``strength``× (clamped to
                                # 0.99 to leave a non-zero exploration
                                # floor for PUCT Q/N at large N).
                                argmax_prob = float(
                                    min(
                                        0.99,
                                        max(
                                ...
                                # Legacy path: floor 0.05 (was 0.5) so
                                # magnitude information can flow; cap
                                # at 1.0.
                                argmax_prob = float(
                                    max(
                                        0.05,
                                        min(1.0, base_argmax_prob),
                                    )
                                )
                            # Attach children FIRST so root.children
                            # exists for the boost loop (the w2swi9tsu
                            # Phase-3 bug — root.children is empty
                            # when this block ran).  Stash the cache
                            # so _simulate reuses these inflated
                            # children instead of creating fresh
                            # nodes with P=0.5.
                            try:
                                self._attach_children(
                                    root, expand_cache
                                )
                                self._root_children_cache_for_search = (
                                    expand_cache
                                )
                            except Exception:
                            ...
                            for child in root.children:
                                try:
                                    child_rule = getattr(
                                        child, "rule_name", None
                                    )
                                    child_tile_smi = ""
                                    try:
                                        ctile = getattr(child, "tile", None)
                                        if ctile is not None:
                                            child_tile_smi = str(
                                                ctile.canonical_smiles()
                                                or ""
                                            )
                                    except Exception:
                                        child_tile_smi = ""
                                    ...
                                    if child_action == argmax_action:
                                        child.P = argmax_prob
                                except Exception:
                                    pass

                            # WF-Deflex Wire-up Phase 3 — Sub-fix B
                            # ``use_learned_prior_argmax`` opt-in.  When
                            # the caller supplies ``learned_prior`` AND
                            # ``use_learned_prior_argmax=True``, we lift
                            # the child whose rule is the learned prior's
                            # argmax by ADDING ``lp_argmax_prob * 0.75``
                            # to ``pocket_prior[argmax_action]`` BEFORE
                            # the boost loop fires (not just clamping
                            # child.P = 0.75 after).  This breaks the
                            # symmetry collapse where every novel pocket
                            # produces the same candidate list — the
                            # learned prior picks the rule most likely
                            # to fire on the *current* state SMILES, and
                            # the pocket-argmax picks the rule most
                            # likely to fire on the *pocket* signature.
                            # The two effects are stacked inside the
                            # ``pocket_prior`` dict so the existing
                            # pocket-argmax boost loop picks up the
                            # learned signal.  When the learned rule
                            # and pocket argmax rule agree (== same
                            # action key), the boost compounds; when
                            # they disagree, the higher-score wins via
                            # the argmax + base_argmax_prob*strength
                            # formula.
                            if (
                                use_learned_prior_argmax
                                and self._learned_prior_for_search
                                is not None
                                and pocket_prior
                            ):
                                try:
                                    lp = self._learned_prior_for_search
                                    try:
                                        state_smi = (
                                            str(
                                                initial_state
                                    ...
                                    if state_smi:
                                        ...
                                        if lp_dist:
                                            ...
                                            lp_argmax_prob = float(
                                                lp_dist[lp_argmax_rule]
                                            )
                                            # Find the action key in
                                            # pocket_prior whose rule
                                            # matches lp_argmax_rule
                                            # (any tile).  The pocket-
                                            # argmax action may use a
                                            # specific tile SMILES —
                                            # match by rule prefix.
                                            lp_action_key = None
                                            for (
                                                pkt_act_key,
                                                pkt_act_val,
                                            ) in pocket_prior.items():
                                                try:
                                                    pkt_rule = (
                                                        pkt_act_key[0]
                                                        if isinstance(
                                                ...
                                                if (
                                                    ...
                                                    # keep first match
                                                    break
                                            if lp_action_key is not None:
                                                # Sub-fix B contract
                                                # (WF-Pocket-Invariance
                                                # Phase 3): add
                                                # ``lp_argmax_prob * 0.75``
                                                # to the pocket_prior entry
                                                # so the boost loop below
                                                # picks up the learned
                                                # signal.  This is the
                                                # ``add learned_prior
                                                # argmax probability * 0.75
                                                # to pocket_prior[argmax_action]``
                                                # contract.  Clamp to 1.0
                                                # so the dataclass invariant
                                                # ``P in [0, 1]`` holds.
                                                pocket_prior[lp_action_key] = (
                                                    min(
                                                        1.0,
                                                        float(
                                                            pocket_prior[
                                                ...
                                                # refresh argmax_action
                                                # for the boost loop below
                                                argmax_action = max(
                                                    pocket_prior,
                                                    key=pocket_prior.get,
                                                )
                                                # refresh base_argmax_prob
                                                # so the new argmax wins
                                                # the boost loop
                                                base_argmax_prob = float(
                                                    pocket_prior[
                                                        argmax_action
                                                ...
                                                # re-derive argmax_prob
                                                # with the same floor logic
                                                strength = float(
                                                    pocket_boost_strength
                                                )
                                                if strength > 1.0:
                                                    argmax_prob = float(
                                                        min(
                        ...
                        # Sticky fallback — never fail search() on a
                        # pocket-injection error.  The legacy root.P
                        # value stands.
                        pass
            except Exception:
                # Sticky fallback — never fail search() on a
                # pocket-injection error.  The legacy root.P value
                # stands.
                pass
        ...
        # WF-Lambda-Fix-FullPath Fix 3 — diversity bonus at rank-time.
        # When ``reward.w_diversity > 0`` we re-score each candidate by
        # adding ``w_diversity * 1.0`` if its canonical SMILES is
        # unique within the candidate set, ``0.0`` otherwise.  This
        # biases the final top-K ordering toward molecules that are
        # *uniquely* represented in the result set, breaking ties
        # when reward scores are similar (and lifting the
        # diversity_tanimoto / diversity_homotype panels without
        # disturbing the dominant Vina / SA / PB reward channels).
        try:
            w_div = float(getattr(reward_fn, "w_diversity", 0.0) or 0.0)
        except Exception:
            w_div = 0.0
        if w_div > 0.0 and candidates:
            state_list = [s for _, s in candidates]
            candidates = [
                (
                    float(score)
                    + w_div * float(
                        reward_fn.diversity_bonus(s, state_list)
                    ),
                    s,
                )
        ...
        # TODO-30 / Rank-1 P3.3 — when ``pareto_rank_top_k`` is True we
        # delegate to :func:`_pareto_rank_candidates` so the ordering
        # uses NSGA-II Pareto rank + crowding distance on the 4-D
        # multi-objective vector (scalar_reward, 1-sa, qed, vina_proxy).
        # Default (``pareto_rank_top_k=False``) preserves the legacy
        # ``sorted(..., key=lambda p: p[0], reverse=True)`` order
        # bit-for-bit (the helper short-circuits to the legacy sort
        # when its internal pareto flag is off).
        if self.pareto_rank_top_k:
            _pareto_rank_candidates(
                candidates,
                reward_fn=reward_fn,
        ...
        if self.leaf_oracle_call_top_k_only and self.oracle_top_k > 0:
            ...
            if oracle is not None:
                ...
                for score, state in candidates[int(self.oracle_top_k):]:
                    filtered.append((score, state))
                candidates = filtered
                # TODO-30 / Rank-1 P3.3 — re-apply the Pareto ranker
                # after the top-K oracle filter so the final ordering
                # still respects ``pareto_rank_top_k``.  When OFF
                # (default) this is bit-for-bit identical to the
                # previous legacy sort.
                if self.pareto_rank_top_k:
                    _pareto_rank_candidates(
                        candidates,
                        reward_fn=reward_fn,
        ...
        result_states = [state for _, state in candidates[: self.top_k]]
        # WF-Lambda-MCTS-Coords-Fix (Step 1-2) — when requested,
        # co-emit a 3-D conformer tensor on each emitted candidate.
        # We attach the tensor as a side-effect attribute (no SMILES /
        # connectivity mutation) so backward-compatible callers are
        # unaffected.  Embedding is best-effort; failures leave the
        # attribute unset and bump the audit counter on the last
        # history entry.
        #
        # TODO-30 / Rank-7 P2.2 (2026-09-17) — safety guard:
        # ``materialize_3d`` defaults to True now.  If RDKit embedding
        # fails at the *global* level (RDKit import fails, ETKDGv3
        # misconfigured, torch missing), we log a single WARNING and
        # fall back to a no-op (same as the previous ``False``
        # behaviour).  Per-candidate failures are still best-effort
        # and continue silently — they only bump ``n_attached``.
        n_attached = 0
        if materialize_3d:
            try:
                from rdkit import Chem as _Chem_local  # type: ignore
                from rdkit.Chem import AllChem as _AllChem_local  # type: ignore
            except Exception as _rdkit_import_exc:
                _Chem_local = None  # type: ignore
                _AllChem_local = None  # type: ignore
                # TODO-30 P2.2 safety guard — RDKit not importable;
                # log a single WARNING (not an error) and fall back to
                # the no-op behaviour (no coords attached).  This
                # keeps ``search()`` functional on RDKit-less
                # environments and on the headless test harness where
                # rdkit is mocked.
                _LOG_P2_2_FALLBACK = getattr(
                    __import__("logging"), "getLogger",
                )(__name__)
                _LOG_P2_2_FALLBACK.warning(
                    "materialize_3d=True requested but RDKit could not "
                    "be imported (%s); falling back to no-op "
                    "(no coords_3d attribute will be attached). "
                    "TODO-30 P2.2 safety guard.",
                    _rdkit_import_exc,
                )
            if _Chem_local is not None and _AllChem_local is not None:
                try:
                    import torch as _torch_local  # type: ignore
                except Exception as _torch_import_exc:
                    _torch_local = None  # type: ignore
                    _LOG_P2_2_TORCH = getattr(
                        __import__("logging"), "getLogger",
                    )(__name__)
                    _LOG_P2_2_TORCH.warning(
                        "materialize_3d=True requested but torch could "
                        "not be imported (%s); falling back to no-op.",
                        _torch_import_exc,
                    )
                if _torch_local is not None:
                    for cand in result_states:
                        try:
                            smi = cand.canonical_smiles()
                        except Exception:
                            smi = None
                        if not smi:
                            continue
                        try:
                            mol = _Chem_local.MolFromSmiles(smi)
                            if mol is None:
                                continue
                            mol_h = _Chem_local.AddHs(mol)
                            params = _AllChem_local.ETKDGv3()
                            rc = _AllChem_local.EmbedMolecule(mol_h, params)
                            if int(rc) != 0:
                                # Embedding failed — leave coords unset.
                                continue
                            mol_no_h = _Chem_local.RemoveHs(mol_h)
                            conf = mol_no_h.GetConformer()
                            n = mol_no_h.GetNumAtoms()
                            coord = conf.GetPositions()
                            # Slice to n_heavy atoms.
                            import numpy as _np  # type: ignore
                            coord_np = _np.asarray(coord[:n], dtype=float)
                            cand.coords_3d = _torch_local.as_tensor(
                                coord_np, dtype=_torch_local.float32,
                            )
                            n_attached += 1
                        except Exception:
                            # Never crash search() on a coords failure.
                            continue
        # Audit counter on the last history entry so downstream reports
        # can report n_attached / n_returned without re-running RDKit.
        if self.history:
            try:
                self.history[-1]["n_coords_3d_attached"] = int(n_attached)
                self.history[-1]["n_coords_3d_returned"] = int(
                    len(result_states)
                )
            except Exception:
                pass

        # WF-T25-SA-Prior — record the SA-prior configuration on the
        # last history entry so downstream reports can reconstruct the
        # search configuration from the per-cell record.  Reset the
        # stash so a subsequent ``search()`` call without
        # ``use_sa_prior=True`` doesn't leak the prior object.
        try:
            if self.history:
                self.history[-1]["use_sa_prior"] = bool(
                    getattr(self, "_use_sa_prior_for_search", False)
                )
                self.history[-1]["sa_prior_strength"] = float(
                    getattr(self, "_sa_prior_strength_for_search", 0.5)
                )
        except Exception:
            pass
        # Keep the SA prior stash so subsequent search() calls in the
        # same Python process retain their config (e.g. Round-12 pilots
        # that loop over pockets in one process).  The flag is read by
        # :meth:`_prior` on every call so a fresh ``search(
        # use_sa_prior=False)`` call cleanly resets it via the stash
    ...
    def _attach_children(self, parent, children) -> None:
        ...
        WF-Pocket-Invariance Combined Phase-2B (sub-fix B)
        ----------------------------------------------------
        When ``self._learned_prior_for_search`` is set on the search
        instance (via ``search(..., learned_prior=..., learned_prior_mix_uniform=...)``),
        the per-child prior is mixed with the learned policy head per
        the AGZ root-noise convention::

            final_P(a|s) = (1 - mix_uniform) * uniform
                          + mix_uniform * learned_prior.predict_proba(s)[a]

        This is the sub-fix B contract: the learned policy head gets a
        non-zero voice at every expansion (not just the root).  With
        ``learned_prior_mix_uniform=0.0`` (pure uniform) the prior
        stays identical to the legacy cascade; with ``mix_uniform=1.0``
        (pure learned) the prior is the learned distribution; with
        ``mix_uniform in (0, 1)`` it is the convex combination.

        The mix is applied per-child AFTER ``_prior(child_state)`` has
        produced the legacy scalar — so every other layer
        (``RewardAggregator`` heuristic, :class:`SymbolicPrior` legacy
        path, constant 0.5 stub) still runs bit-for-bit.  This
        preserves full backward compatibility (default ``mix_uniform=0.5``
        in the w2swi9tsu Phase-3 patch was a no-op because
        ``_learned_prior_for_search`` was None — this Phase-2B
        ...
        # WF-Pocket-Invariance Phase-2B — pre-compute the learned prior
        # distribution for the parent state once, so every child of
        # this parent consults the same ``learned_prior.predict_proba``
        # output (the head is O(1) per call but the dict materialise
        # is non-trivial — caching it here is bit-cheap).
            ...
            try:
                ...
                try:
                    if parent is not None and getattr(parent, "state", None) is not None:
                        parent_smi = str(
                            parent.state.canonical_smiles() or ""
                        )
                except Exception:
                    parent_smi = ""
        ...
        for child_state, rule_name, tile in children:
            ...
            # learned-prior mix.  Per-child (state, action) lookup:
            # the action is ``rule_name`` (the click rule that fired to
            # produce this child).  The learned prior gives a
            # rule-indexed dict for the *parent* state, which is the
            # canonical input the head was trained on.  Mix with
            # uniform per the AGZ root-noise convention
            # (Silver 2017 Eq. 2)::
            #
            #     P_mix(a|s) = (1 - mix_uniform) * U(a) + mix_uniform * learned_prior(a|s)
            #
            # where ``U(a) = 1 / n_rules`` (uniform over the click-rule
            # subset).  At ``mix_uniform=0.0`` the prior is pure
            # uniform (legacy fallback); at ``mix_uniform=1.0`` the
            # prior is pure learned; intermediate values are the
            # convex combination.
            if learned_dist_cache and mix_uniform > 0.0:
                try:
                    ...
                    # Defensive fallback — never fail child attach on
                    # a learned-prior mix error.  Legacy P stands.
                    pass
    ...
    def _prior(self, state: MoleculeClosedTerm) -> float:
        ...
        WF-T25-SA-Prior (2026-09-17)
        ----------------------------
        When ``search(use_sa_prior=True)`` is set, the resulting
        prior mass is *additively blended* with an SA-aware mass
        derived from the candidate SMILES:

            p_final = (1 - β) · p_legacy + β · p_sa(x)

        where ``β = sa_prior_strength`` (default 0.5) and
        ``p_sa(x) = sigmoid( -log(sa(x)) - log_baseline )`` with
        ``log_baseline = -log(MOSES_median)``.  When
        ``use_sa_prior=False`` (default) the prior is bit-for-bit
        identical to the legacy behaviour — every branch below
        returns the same float as before the wire-up.
        """
        ...
        if legacy_p == 0.5 and self.prior is not None:
            try:
                legacy_p = float(self.prior.predict_proba(state))
            except Exception:
                pass
        ...
        if legacy_p == 0.5:
            try:
                ...
                legacy_p = float(heuristic(features))
            except Exception:
                legacy_p = 0.5

        # WF-T25-SA-Prior — additive blend with the SA-aware prior.
        # When the gate is off (the default), the original ``legacy_p``
        # is returned unchanged so the search is bit-for-bit identical
        # to the pre-wire-up behaviour.  When the gate is on, mix the
        # legacy mass with the SA-derived mass in linear space (a
        # smooth interpolation; the legacy behaviour is recovered at
        # ``sa_prior_strength=0`` and the pure SA-aware prior at
        # ``sa_prior_strength=1``).
        try:
            use_sa = bool(getattr(self, "_use_sa_prior_for_search", False))
        except Exception:
            use_sa = False
        if not use_sa:
            return float(max(1e-6, min(1.0 - 1e-6, legacy_p)))
        ...
        # Lazy SAPrior construction — single instance per search.
        sa_prior_obj = getattr(self, "_sa_prior_obj_for_search", None)
        if sa_prior_obj is None:
            try:
                from molmetal_lam.search_alg.priors import SAPrior
                sa_prior_obj = SAPrior()
                self._sa_prior_obj_for_search = sa_prior_obj
            except Exception:
                # Module unavailable — degrade to the legacy prior.
                return float(max(1e-6, min(1.0 - 1e-6, legacy_p)))

        # Resolve SMILES from the state — same fallbacks as
        # :meth:`heuristic`.
        smi = ""
        try:
            fn = getattr(state, "canonical_smiles", None)
        ...
        # SA-derived prior mass in [0, 1].  When the SMILES is
        # unparseable / no SA backend is available, the SAPrior
        # returns 0.5 (uniform baseline) so the blended mass is
        # unchanged.
        try:
            p_sa = float(sa_prior_obj.prior_mass(smi))
        except Exception:
            p_sa = 0.5
        if not math.isfinite(p_sa):
            p_sa = 0.5

        blended = float((1.0 - beta) * legacy_p + beta * p_sa)
        return float(max(1e-6, min(1.0 - 1e-6, blended)))
    ...
    def _safe_reduce(
        ...
        ----------------------------
        For the three SMARTS-defined click rules
        (:class:`CuAAC`, :class:`SPAAC`, :class:`Suzuki`) we additionally
        retry with the arguments swapped.  The underlying chemistry is
        commutative (an azide + alkyne reacts regardless of order) but
        the SMARTS templates are written with the azide/boronic-acid
        handle as the *first* reactant.  Without the swap, MCTS roots
        whose state is the alkyne (e.g. cisplatin-acetylide ``[Pt]C#C``)
        silently fail to fire CuAAC even when the partner tile carries
        the azide.  See ``WF-Lambda-Rule-Symmetry-Fix`` in
        ``TODO/pending/`` and the closure root-cause analysis in
        ``molmetal/reports/wf_partner_tiles_patha/final.md`` §1.

        For all other rules (SPC, ThiolEne, AmideCoupling,
        MetalLigandExchange, AquaExchange, DielsAlder) the rule's
        own ``_reduce`` handles the asymmetry if any; we just call
        ``rule.reduce((state, tile))`` once and trust the result.

        TODO-30 P1.2 — when ``self.fg_veto_strict`` is True, every
        ``rule.reduce`` call carries the ``fg_constraints={"strict":
        True}`` kwarg so :class:`ReactionRule.reduce` consults the
        FG-compatibility table and silently returns ``[]`` on veto.
        Default (False) preserves bit-for-bit backward compat.
        """
        # TODO-30 P1.2 — opt-in FG veto kwarg plumbed to ReactionRule.reduce
        fg_kwargs = (
            {"fg_constraints": {"strict": True}}
            if getattr(self, "fg_veto_strict", False)
            else {}
        )
        try:
            forward = rule.reduce((state, tile), **fg_kwargs)
        except Exception:
            forward = []
        if forward:
            return forward
        # Symmetry retry: only for the SMARTS-defined click rules whose
        # SMARTS is explicitly asymmetric.  We key off the rule's name
        # so adding more rules doesn't require touching this list.
        if getattr(rule, "name", "") in _SYMMETRIC_RULE_NAMES:
            try:
                swapped = rule.reduce((tile, state), **fg_kwargs)
            except Exception:
                swapped = []
            return swapped
        return []
